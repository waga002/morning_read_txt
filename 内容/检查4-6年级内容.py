...
def check_logic_issues(content_item, file_ref, grade):
    """检查内容逻辑问题"""
    issues = []
    ...
    if len(sentences) >= 2:
        ...
        for i in range(len(sentences) - 1):
            ...
            # 跳过注释内容（括号内的内容）
            if sent1.startswith('（') or sent1.startswith('('):
                continue
            if sent2.startswith('）') or sent2.startswith(')'):
                continue
            
            # 检查是否有明显的主题切换（可能是硬凑）
            common_chars = set(sent1) & set(sent2)
            if len(common_chars) == 0 and len(sent1) < 20 and len(sent2) < 20:
                # 进一步检查：是否是完全不同的主题
                animal_keywords = ['猫', '狗', '鸟', '鱼', '兔', '熊', '虎', '狮', '象', '猴', '鸡', '鸭', '鹅', '蝉', '牧童', '鲨鱼']
                color_keywords = ['红', '绿', '蓝', '黄', '白', '黑', '紫', '橙', '碧', '雪', '翠', '金黄']
                weather_keywords = ['雨', '雪', '风', '云', '太阳', '月亮', '星星', '晴天', '阴天', '潮', '浪']
                plant_keywords = ['花', '草', '树', '叶', '莲', '荷', '梧桐', '绒球花']
                school_keywords = ['学校', '校园', '教室', '操场', '老师', '同学', '学生']
                nature_keywords = ['山', '海', '江', '河', '湖', '水', '天', '地']
                ...
                sent1_has_nature = any(kw in sent1 for kw in nature_keywords)
                ...
                sent2_has_nature = any(kw in sent2 for kw in nature_keywords)
    ...
    if not (content_type == 'daily_accumulation' and is_sentence_pattern):
        ...
        if len(time_found) > 1 and len(full_text) < 100:
    ...
    if content_type not in ['tongue_twisters']:
        ...
        if has_positive and has_negative and len(full_text) < 60:
            # 检查是否是明显的对比结构（如"是...不是..."）
            if not (('是' in full_text and '不是' in full_text) or 
                ...
                issues.append({
                    ...
                    "file": file_ref,
                    "title": title,
                    "problem": f"短文本中同时出现肯定和否定，可能存在矛盾：{full_text[:100]}"
                })
    
    # 4. 检查内容是否过于简短或不完整
    if content_type in ['modern_prose', 'story_legend', 'textbook_review']:
        # 根据年级调整最短长度要求
        min_length = {4: 40, 5: 50, 6: 60}.get(grade, 40)
        min_sentences = {4: 3, 5: 3, 6: 4}.get(grade, 3)
        if len(full_text) < min_length and len(sentences) < min_sentences:
            # 排除古诗复习
            if '复习' not in title or content_type != 'ancient_poem':
                issues.append({
                    "type": "内容过于简短",
                    "file": file_ref,
                    "title": title,
                    "problem": f"内容过于简短，可能不够完整：{full_text[:100]}"
                })
    ...
    if content_type in ['modern_prose', 'textbook_review']:
        # 4-6年级可以接受更复杂的词汇，只检查明显不适合的
        if grade == 4:
            complex_words = ["哲学", "理论", "抽象", "概念", "原理", "机制", "系统", "深奥", "高深", "专业术语", "量子", "相对论"]
        elif grade == 5:
            complex_words = ["量子", "相对论", "哲学体系", "抽象思维", "理论框架"]
        else:  # grade == 6
            complex_words = ["量子力学", "相对论", "哲学体系", "抽象思维", "理论框架", "高深莫测"]
        ...
        for word in complex_words:
            if word in full_text:
                issues.append({
                    ...
                    "file": file_ref,
                    "title": title,
                    "problem": f"内容可能过于复杂，不适合{grade}年级学生：包含词汇'{word}'"
                })
                break
        ...
        if '这是' in annotation or '表达' in annotation or '说明' in annotation:
            if not re.search(r'[：:].*[。！]', annotation):  # 如果不是字词解释格式
                issues.append({
                    ...
                    "file": file_ref,
                    "title": title,
                    "problem": f"注解应只包含字词解释，不应包含说明性句子：{annotation[:150]}"
                })
    ...
    if task and task_answer:
        ...
        if '？' in task or '?' in task or '什么' in task or '怎么' in task or '为什么' in task or '哪里' in task or '如何' in task:
            # task是问句，taskAnswer应该给出答案
            min_answer_length = {4: 10, 5: 15, 6: 20}.get(grade, 10)
            if len(task_answer) < min_answer_length:
                issues.append({
                    "type": "任务答案不匹配",
                    "file": file_ref,
                    "title": title,
...
def check_grade_content(grade):
    """检查指定年级的内容"""
    base_dir = Path(__file__).parent
    grade_dir = base_dir / "无拼音" / f"{grade}年级"
    
    if not grade_dir.exists():
        print(f"错误：未找到{grade}年级内容路径: {grade_dir}")
        return {}
        ...
        try:
            ...
            for week_data in data:
                ...
                for content_item in week_data.get('content', []):
                    ...
                    # 检查逻辑问题
                    logic_issues = check_logic_issues(content_item, file_ref, grade)
...
def print_report(issues, grade):
    """打印检查报告"""
    print("=" * 60)
    print(f"{grade}年级内容检查报告")
    print("=" * 60)
...
def save_report_to_file(issues, output_file, grade):
    """保存详细报告到文件"""
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(f"# {grade}年级内容检查报告\n\n")
        f.write("## 概述\n")
        f.write(f"本报告全面检查{grade}年级所有教学内容文件，包括内容逻辑检查和合理性检查。\n\n")
        ...
        f.write(f"6. **调整难度不匹配的内容**：对于过于复杂的内容，考虑简化表达，或替换为更符合{grade}年级学生认知水平的简单句式和词语。\n")
...
if __name__ == "__main__":
    ...
    for grade in [4, 5, 6]:
        print(f"\n{'='*80}\n")
        issues = check_grade_content(grade)
        print_report(issues, grade)
        
        # 保存详细报告到文件
        report_file = base_dir / f"{grade}年级内容检查报告.md"
        save_report_to_file(issues, report_file, grade)
