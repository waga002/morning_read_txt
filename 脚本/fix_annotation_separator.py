...
# -*- coding: utf-8 -*-
"""
脚本功能：修复JSON文件中annotation字段的分隔符
将所有的 '!!' 替换为 '！！'
例如："annotation": "注释1。!!注释2。" 修改为 "annotation": "注释1。！！注释2。"
"""
...
def process_json_file(file_path):
    """
    处理单个JSON文件，将annotation字段中的'!!'替换为'！！'
    """
...
def process_text_file(file_path):
    ...
    def replace_in_annotation(match):
        ...
        if '!!' in annotation_value and '！！' not in annotation_value:
            new_value = annotation_value.replace('!!', '！！')
            modified_count += 1
            return f'"annotation": "{new_value}"'
        elif '!!' in annotation_value:
            # 如果同时存在，只替换 '!!'
            new_value = annotation_value.replace('!!', '！！')
            modified_count += 1
            return f'"annotation": "{new_value}"'
    ...
    # 替换所有匹配的内容
    new_content = re.sub(pattern, replace_in_annotation, content)
